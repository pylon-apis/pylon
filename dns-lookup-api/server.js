...
async function x402PaymentCheck(req, res, next) {
  ...
  try {
    const verifyRes = await fetch(`${FACILITATOR_URL}/verify`, {
      ...
      body: JSON.stringify({ payload: paymentHeader, details: { scheme: "exact", network: NETWORK, amount: PRICE_AMOUNT, asset: "0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913", resource: req.originalUrl, description: "DNS lookup", payTo: WALLET_ADDRESS, maxTimeoutSeconds: 60, outputSchema: null, extra: { name: "USDC", version: "2" } } }),
    });
...
function settlePayment(req) {
  ...
  fetch(`${FACILITATOR_URL}/settle`, {
    method: "POST", headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ payload: req.x402Payment, details: { scheme: "exact", network: NETWORK, amount: PRICE_AMOUNT, asset: "0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913", resource: req.originalUrl, description: "DNS lookup", payTo: WALLET_ADDRESS, maxTimeoutSeconds: 60, outputSchema: null, extra: { name: "USDC", version: "2" } } }),
